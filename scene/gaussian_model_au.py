...
class GaussianModelAU: 
    ...
    def capture(self):
        return (
            ...
            {k: v.clone().detach().cpu() for k, v in self.offsets.items()},  # save offsets dict
            (self.mr_list.clone().detach().cpu() if hasattr(self, "mr_list") else None),  # save mr_list or None
        )
    ...
    def restore(self, model_args, training_args):
        ...
        self.offsets = {k: v.clone().detach().to(self._xyz.device).requires_grad_(True) for k, v in offsets.items()}
        if mr_list is not None:
            self.mr_list = mr_list.clone().detach().to(self._xyz.device).requires_grad_(False)

        # --- Now safe to call training_setup() ---
    ...
    def sample(self):
        """Sample from the learned distributions of position, scale, opacity"""
        import copy
        sampled = copy.deepcopy(self)
        with torch.no_grad():
            # Sample model IDs for mixture model
            sample_model_ids = torch.randperm(self.n_models)[:self.M].cuda()

            # === Position ===
            std = self.offsets["_xyz_offset"][..., sample_model_ids].mean(dim=-1)
            std = torch.nn.functional.softplus(std)
            mean = self.offsets["_xyz_offset"][..., sample_model_ids + self.n_models].mean(dim=-1)
            xyz_noise = torch.randn_like(self._xyz)
            sampled._xyz = self._xyz + self.mr_list * (xyz_noise * std + mean)

            # === Scaling ===
            width = self.offsets["_scaling_offset"][..., sample_model_ids].mean(dim=-1)
            width = torch.nn.functional.softplus(width).clamp_(1e-2, 1e2)
            left = self.offsets["_scaling_offset"][..., sample_model_ids + self.n_models].mean(dim=-1)
            offset_scal = width * torch.rand_like(self._scaling) + left
            offset_scal = self.mr_list * offset_scal + (1 - self.mr_list)
            sampled._scaling = self._scaling * offset_scal

            # === Opacity ===
            std = self.offsets["_opacity_offset"][..., sample_model_ids].mean(dim=-1)
            std = torch.nn.functional.softplus(std)
            mean = self.offsets["_opacity_offset"][..., sample_model_ids + self.n_models].mean(dim=-1)
            p_logit = (std * torch.randn_like(self._opacity) + mean)
            offset_opc = torch.sigmoid(p_logit / self.tmp)
            offset_opc = self.mr_list * offset_opc + (1 - self.mr_list)
            sampled._opacity = self._opacity * offset_opc
